...
extern "C" {
    ...
    fn call_init(size: *const glue::Bounds, closure_data: *const u8, output: *mut Model);
    ...
    fn call_render(model: *const Model, closure_data: *const u8, output: *mut RocList<glue::Elem>);
...
pub fn init_and_render(bounds: glue::Bounds) -> (*const Model, RocList<glue::Elem>) {
    ...
    let elems = unsafe {
        let mut ret_val: MaybeUninit<RocList<glue::Elem>> = MaybeUninit::uninit();
...
pub fn update(model: *const Model, event: glue::Event) -> *const Model {
    let closure_data_buf;
    let closure_layout;
...
/// Call the app's update function, then render and return that result
pub fn update_and_render(model: *const Model, event: glue::Event) -> (*const Model, RocList<glue::Elem>) {
    let closure_data_buf;
    let closure_layout;
